...
export const animations = () => {
  ...
  t5.add({
    ...
    duration: "1000",
    targets: ".track-ml",
    translateY: device === "xs" ? "20em" : "13em",
    opacity: "1",
  })
    .add(
      {
        delay: device === "xs" ? "0" : "1000",
        targets: ".track-ARVR",
        keyframes: [{ translateY: device === "xs" ? "29em" : "25em" }],
        opacity: "1",
      },
      600
    )
    .add(
      {
        delay: device === "xs" ? "0" : "1000",
        targets: ".track-BC",
        keyframes: [{ translateY: device === "xs" ? "38em" : "39em" }],
        opacity: "1",
      },
      800
    )
    .add(
      {
        delay: device === "xs" ? "0" : "1000",
        targets: ".track-OI",
        keyframes: [{ translateY: device === "xs" ? "47em" : "45em" }],
        opacity: "1",
      },
      1200
    );
...
export const goBack = (selector, firstRow) => {
  if (selector.includes("mobile")) {
    ...
    if (firstRow) {
      const section = homeSections.firstRow.filter(
        (s) => s.class === selector
      )[0];
      section.reverseAnimation("reverse");
    } else {
      const section = homeSections.secondRow.filter(
        (s) => s.class === selector
      )[0];
      section.reverseAnimation("reverse");
    }
