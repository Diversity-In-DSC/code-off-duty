...
export const trackAnimations = () => {
	...
	t5
		.add({
			...
			duration: '1000',
			targets: '.track-ml',
			translateY: device === 'xs' ? '18em' : '13em',
			opacity: '1'
		})
		.add(
			{
				...
				targets: '.track-ARVR',
				keyframes: [ { translateY: device === 'xs' ? '27em' : '25em' } ],
				opacity: '1'
			},
			600
		)
		.add(
			{
				delay: device === 'xs' ? '0' : '1000',
				targets: '.track-BC',
				keyframes: [ { translateY: device === 'xs' ? '36em' : '39em' } ],
				opacity: '1'
			},
			800
		)
		.add(
			{
				delay: device === 'xs' ? '0' : '1000',
				targets: '.track-OI',
				keyframes: [ { translateY: device === 'xs' ? '45em' : '45em' } ],
				opacity: '1'
			},
			1200
		);
...
export const getOtherSections = (selector) => {
	...
	Array.prototype.push.apply(rest, [
		...
		document.querySelector('#timeline-btn')
	]);
	if (device === 'xs') {
